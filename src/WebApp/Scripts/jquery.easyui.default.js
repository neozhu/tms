...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    mobile: {// 验证手机号码  
        validator: function (value) {
            return /^(13|15|18|17|16|19)\d{9}$/i.test(value);
        },
        message: '手机号码格式不正确'
    },
    idcard: {// 验证身份证  
  ...
  decimal: {
    validator: function (value) {
      ...
      return /^\s*(?=.*[1-9])\d*(?:\.\d{1,8})?\s*$/.test(value);
    },
    message: '不能小于等于[0]'
  },
  minLength: {//最小字符数
...
function numberformatter8(value) {
  if (value == null || value == '') {
    return value;
  } else {
...
function numberformatter5(value) {
  if (value == null || value == '') {
    return value;
  } else {
