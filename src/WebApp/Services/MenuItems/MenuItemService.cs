  ...
  public class MenuItemService : Service<MenuItem>, IMenuItemService
  {
    ...
    public void ImportDataTable(System.Data.DataTable datatable)
    {
      foreach (DataRow row in datatable.Rows)
      {
        ...
        var mapping = this._mappingservice.Queryable().Where(x => x.EntitySetName == "MenuItem" && ( ( x.IsEnabled == true ) || ( x.IsEnabled == false && !( x.DefaultValue == null || x.DefaultValue.Equals(string.Empty) ) ) )).ToList();
    ...
    public Stream ExportExcel(string filterRules = "", string sort = "Id", string order = "asc")
    {
      ...
      var menuitems = this.Query(new MenuItemQuery().Withfilter(filters)).Include(p => p.Parent).OrderBy(n => n.OrderBy(sort, order)).Select().ToList();
